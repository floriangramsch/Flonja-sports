...
<template>
  ...
  <!-- Exercises List -->
  <SlideTransition>
    <div v-if="workoutShow.showRouter === 'workoutdetail'" class="absolute inset-0">
      <!-- Header -->
      <div class="flex w-full justify-evenly px-2 py-4">
        <!-- End Workout -->
        <Confirm v-model:isOpen="showConfirmEndWorkout" @yes="endWorkout"
        ...
        <Dialog :isOpen="showLockerDialog" @close="showLockerDialog = false">
          <div class="flex flex-col items-center justify-center gap-4">
            <div class="flex gap-2">
              <UiNumberInput v-model:modelValue="newLocker" label="Lockernummer" :focus="!workout?.locker" />
            </div>
            <Button @action="updateWorkout"> Done </Button>
          </div>
        </Dialog>
      </div>
      <div v-if="exercises?.length !== 0" v-for="ex in exercises"
      ...
      <div v-else>
        <div class="flex h-20 w-full cursor-pointer items-center justify-center bg-sonja-bg-darker text-3xl font-bold"
          @click="() => (workoutShow.showRouter = 'equipselection')">
          Start Workout
        </div>
      </div>
      <button class="flex w-full justify-center rounded-full rounded-t bg-sonja-bg-darker pb-2 pt-3"
        @click="() => (workoutShow.showRouter = 'equipselection')">
