...
<template>
  ...
  <SlideTransition>
    <div v-if="showMuscleOverview">
      ...
      <div class="inset-0 grid grid-cols-3 place-items-center gap-2 overflow-auto">
        <div
          class="flex size-28 cursor-pointer items-center justify-center overflow-auto border-4 border-sonja-bg-darker"
          @click="chooseMuscle(muscle)" v-for="muscle in muscles" :key="muscle.muscle_group_id">
          {{ muscle.muscle_name }}
        </div>
  ...
  <SlideTransition>
    <div v-if="showEquipOverview">
      <div class="flex w-full justify-evenly py-4">
        <button class="flex h-10 items-center rounded-full bg-sonja-bg-darker px-4 text-sonja-text shadow" @click="
      ...
      <div class="absolute inset-0 mt-16 flex flex-col">
        <div v-for="equip in equipsToShow" @click="addNewExercice(Number(equip.equip_id))"
        ...
        <Dialog :isOpen="showDialogEquip" @close="showDialogEquip = false">
          ...
          <NewEquip @close="showDialogEquip = false" :muscleId="chosenMuscle?.muscle_group_id" />
        </Dialog>
      </div>
    </div>
  </SlideTransition>
